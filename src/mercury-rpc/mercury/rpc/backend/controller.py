...
from mercury.rpc.active_asyncio import active_state, add_active_record
...
class BackendController(StaticEndpointController):
    ...
    async def register(self, device_info, agent_info):
        ...
        log.debug('Created/Updated inventory record: %s' % response['object_id'])
    ...
    async def update(self, mercury_id, update_data):
        ...
        if mercury_id not in active_state:
            raise MercuryClientException('Received update for inactive device: {}'.format(mercury_id))
    ...
    async def update_task(self, update_data):
        ...
        TODO: enforce task_id by requiring that as a positional argument
